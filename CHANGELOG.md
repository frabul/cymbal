...
- **C/C++ struct, union, and class members are now indexed as symbols, and member accesses emit refs** — `field_declaration` members (`const char *name;`, `int x, y;`, bitfields, function-pointer fields like `void (*cb)(int);`) become `field` symbols parented to the enclosing struct tag, union tag, typedef name (for anonymous `typedef struct { … } Name;`), or C++ class — so `outline` shows them, `show Type.member` resolves them, and `investigate` treats them as fields. C++ in-class method declarations (`void decl();`) classify as `method`; function-pointer *data* members stay fields. Named `union` specifiers are now indexed too (kind `union`, ranked with the type kinds). On the reference side, `extractRefC` records a `use` ref for the accessed member of every `s.f` / `p->f` (`field_expression`) and for designated initializers (`{ .value = 1 }`, `field_designator`), so `cymbal refs value` finds all member-access sites; `p->f(x)` does not double-emit (the callee `field_expression` defers to the existing call-ref normalization).
...
- **C / C++: index global variables, function prototypes, and complex typedefs** — `classifyC` now handles the `declaration` node (file-scope `int g = 0;`, `extern int errno;`, prototypes like `extern int printf(const char *, ...);`) and the `type_definition` polymorphic `declarator` field (function-pointer, array, pointer, parenthesized wrappers). Two new helpers: `cIsFileScope` restricts to translation-unit / namespace / linkage-specification scope, and `cDeclaratorName` peels the declarator wrappers down to the leaf identifier. Function-body locals stay unindexed by policy. C++ uses the same classifier, so namespace-scope variables are also covered.
